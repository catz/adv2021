...
def visit_small_twice path
  repeats = path.select{|s| s.downcase == s}.tally.values.tally
  ...
  # remaining small caves can be visited at most once. However, the caves named start
  # and end can only be visited exactly once each: once you leave the start cave, you may not
  # return to it, and once you reach the end cave, the path must end immediately.
  repeats[3] && repeats[3] > 0 || repeats[2] && repeats[2]>1
end
